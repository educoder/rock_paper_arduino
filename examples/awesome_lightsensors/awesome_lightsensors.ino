...
int rockSensor = A1;
int paperSensor = A2;
int scissorsSensor = A3;

int sensorVal = 0;
bool sendChoice = true;
//int sensorValPaper = 0;
//int sensorValScissors = 0;
...
void loop() {
  ...
  if((sensorVal = analogRead(rockSensor)) > 1000) {
    if (sendChoice) {
      rpa.choose_ROCK();
      sendChoice = false;
    }
  } else if((sensorVal = analogRead(paperSensor)) > 1000) {
    if (sendChoice) {
      rpa.choose_PAPER();
      sendChoice = false;
    }
  } else if((sensorVal = analogRead(scissorsSensor)) > 1000) {
    delay(10);
    if (sendChoice) {
      rpa.choose_SCISSORS();
      sendChoice = false;
